/* 먼저 simple sudoku solver로 하나짜리 전부 채우고 경우의 수를 따질 예정
하나의 경우의 수에서 계속해서 경우의 수가 형성이 됨.
하나의 경우의 수에서 스도쿠 완성되면 종료, 모순이 발생하면 다른 경우의 수로 넘어가야 함.
하나의 경우의 수에서 나온 모든 경우의 수가 막히면 그 하나의 경우의 수도 안되는 것이고 다른 경우의 수로 넘어가야 함.
*/
/* 자료구조 이런거랑 연관 있는듯. 일단 내 생각만으로 완성해 보고 그 다음 조사.*/
...
function solvingSimple() {
    ...
    let key = 0;
     ...
     while(blankCounter > 0 && key < 1000 /*simple sudoku 방법으로 어디까지 채워지나 확인*/) {
